...
# ----------------- Gamification (Level + Achievements + Weekly History) -----------------
LEVELS = [
...
def _compute_xp(docs: List[dict]) -> int:
    total = 0
    for d in docs:
        total += XP_PER_OS
        elapsed = int(d.get("execution_elapsed_sec") or 0)
...
def _level_from_xp(xp: int) -> dict:
    ...
    return {**current, "xp": xp, "xp_current_level": xp_in, "xp_next_level": xp_req, "progress_pct": round(min(xp_in / xp_req, 1.0), 3), "next": nxt}


# Catálogo de conquistas (calculadas do zero a cada GET)
def _compute_achievements(docs: List[dict]) -> List[dict]:
    n = len(docs)
    by_empresa = set()
    approved = 0
    fast = 0
    ...
    for d in docs:
        ...
        if 0 < elapsed < SLA_FAST_SEC:
            fast += 1
        base = _base_price(d.get("empresa", ""), d.get("tipo_atendimento") or "Instalação")
        bonus = _sla_bonus(base, elapsed)
        total_net += base + bonus
...
def _compute_weekly_history(docs: List[dict], weeks: int = 8) -> List[dict]:
    now = datetime.now(timezone.utc)
...
@api.get("/gamification/profile", response_model=GamificationOut)
async def gamification_profile(user=Depends(get_current_user)):
    ...
    ach = _compute_achievements(docs)
    weekly = _compute_weekly_history(docs, weeks=8)
